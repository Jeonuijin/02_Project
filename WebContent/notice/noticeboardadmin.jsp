...
<hr>
		...
		<table>
			<thead>
				<tr>
					...
					<th>번호</th>
					<th>구분</th>
					<th class="title">제목</th> <!-- "title" 클래스 추가 -->
					<th>작성일</th>
				</tr>
			...
			<c:forEach var="vo" items="${list}"> 
				...
				<tr>
					<td><input type="checkbox" name="selectedItems" value="${vo.nbIdx}"></td>
					<td>${vo.rnum}</td>
					<td>${vo.type}</td>
		  ...
		  <tfoot>
			<tr>
				<td colspan="4">
					...
					<c:if test="${pvo.beginPage != 1 }">
						<li>
							<a href="noticeboardadmin?cPage=${pvo.beginPage - 1 }"><<</a>
						</li>
					</c:if>	
					...
					<c:if test="${cPage != 1 && cPage != null}">
						<li>
							<a href="noticeboardadmin?cPage=${cPage - 1 }"><</a>
						</li>
					</c:if>	
					...
					<c:if test="${pageNo != pvo.nowPage }">
						<li>
							<a href="noticeboardadmin?cPage=${pageNo }&size=${size}">${pageNo }</a>
						</li>
					</c:if>		
					...
					<c:if test="${cPage != pvo.totalPage}">
						<li>
							<a href="noticeboardadmin?cPage=${cPage + 1 }&size=${size}">></a>
						</li>
					</c:if>	
					...
					<c:if test="${pvo.endPage < pvo.totalPage }">
						<li>
							<a href="noticeboardadmin?cPage=${pvo.endPage + 1 }&size=${size}">>></a>
						</li>
					</c:if>		 
					</ol>
	...
	<div class ="button2">
		<input type="submit" id="deleteButton" value="삭제하기">
		</div>
	</div>
